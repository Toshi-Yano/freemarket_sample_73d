...
mens_grandchild_array = [
                            ...
                            ['長財布','折り財布','マネークリップ','コインケース/小銭入れ','名刺入れ/定期入れ','キーケース','キーホルダー','ネクタイ','手袋','ハンカチ','ベルト','マフラー','ストール','バンダナ','ネックウォーマー','サスペンダー','ウォレットチェーン','サングラス/メガネ','モバイルケース/カバー','手帳','ストラップ','ネクタイピン','カフスリンクス','イヤマフラー','傘','レインコート','ミラー','タバコグッズ','その他'], 
                            ['腕時計(アナログ)','腕時計(デジタル)','ラバーベルト','レザーベルト','金属ベルト','その他'], 
...
baby_kids_grandchild_array = [
                            ...
                            ['トップス','アウター','パンツ','おくるみ','下着/肌着','パジャマ','ロンパース','その他'], 
                            ['コート','ジャケット/上着','トップス(Tシャツ/カットソー)','トップス(トレーナー)','トップス(チュニック)','トップス(タンクトップ)','トップス(その他)','スカート','パンツ','ワンピース','セットアップ','パジャマ','フォオーマル/ドレス','和服','浴衣','甚平','水着','その他'], 
                            ['コート','ジャケット/上着','トップス(Tシャツ/カットソー)','トップス(トレーナー)','トップス(その他)','パンツ','セットアップ','パジャマ','フォオーマル/ドレス','和服','浴衣','甚平','水着','その他'], 
                            ['コート','ジャケット/上着','トップス(Tシャツ/カットソー)','トップス(トレーナー)','トップス(その他)','ボトムズ','パジャマ','その他'],
...
# インテリア・住まい・小物
interior_house_accessory_child_array = ['キッチン/食器','ベッド/マットレス','ソファ/ソファベッド','椅子/チェア','机/テーブル','収納家具','ラグ/カーペット/マット','カーテン/ブラインド','ライト/証明','寝具','インテリア小物','季節/年中行事','その他']
interior_house_accessory_grandchild_array = [
                            ['食器','調理器具','収納/キッチン雑貨','弁当用品','カラトリー(スプーン等)','テーブル用品','容器','エプロン','アルコールグッズ','浄水機','その他'], 
                            ...
                            ['一般','スツール','ダイニングチェア','ハイバックチェア','ロッキングチェア','座椅子','折り畳み椅子','デスクチェア','その他'], 
...
interior_house_accessory_child_array.each_with_index do |child, i|
  child = parent.children.create(name: child)
  interior_house_accessory_grandchild_array[i].each do |grandchild|
    child.children.create(name: grandchild)
  end
end
...
## おもちゃ・ホビー・グッズ
toy_hobby_child_array = ['おもちゃ','タレントグッズ','コミック/アニメグッズ','トレーディングカード','フィギュア','楽器/器材','コレクション''ミリタリー','美術品','アート用品','その他']
toy_hobby_grandchild_array = [
                            ...
                            ['エレキギター','アコースティックギター','ベース','エフェクター','アンプ','弦楽器','管楽器','鍵盤楽器','打楽器','和楽器','楽譜/スコア','レコーディング/PA器材','DJ楽器','DTM/DAW','その他'], 
...
toy_hobby_child_array.each_with_index do |child, i|
  child = parent.children.create(name: child)
...
# 家電・スマホ・カメラ
electrical_appliances_child_array = ['スマートフォン/携帯電話', 'スマホアクセサリー', 'PC/タブレット', 'カメラ', 'テレビ/映像機器', 'オーディオ機器', '美容/健康', '冷暖房/空調', '生活家電', 'その他']

electrical_appliances_grandchild_array = [
...
electrical_appliances_child_array.each_with_index do |child, i|
  child = electrical_appliances.children.create(name: child)
  electrical_appliances_grandchild_array[i].each do |grandchild|
    child.children.create(name: grandchild)
  end
end

# スポーツ・レジャー
sporting_goods_child_array = ['ゴルフ', 'フィッシング', '自転車', 'トレーニング/エクササイズ', '野球', 'サッカー/フットサル', 'テニス', 'スノーボード', 'スキー', 'その他スポーツ', 'アウトドア', 'その他']

sporting_goods_grandchild_array = [
